...
static HostServices* sin_host_services_ = nullptr;
...
extern "C" __declspec(dllexport) int get_extension_metadata(ExtensionMeta** output_info) {
    ...
    // Additional names for the sin function
    ...
    sin_extension_info.priority_level = 4; // Highest priority for functions
                                   ...
                                   char* error_text, size_t error_buffer_size) -> double
    {
        ...
        // Convert degrees to radians and compute sin
...
extern "C" __declspec(dllexport) int initialize_extension(const HostServices* services, 
    ...
    sin_host_services_ = const_cast<HostServices*>(services);
